...
One can integrate the Drop protocol in several ways:
...
They allow to make staking and unstaking with the assets that are already on the Neutron chain (i.e. .
...
One can use messages templates provided in this doc to send them directly to the contracts.
This is especially useful in the case of issuing a transaction from another chain.
For example, ATOM staking can be done in one IBC-transfer that calls Drop contract via [ibc-hooks module on Neutron](https://docs.neutron.org/neutron/modules/3rdparty/osmosis/ibc-hooks/overview).
...
As one can see, staking from Osmosis besides `ibc-hook` module requires using of [pfm](https://github.com/strangelove-ventures/packet-forward-middleware), because ATOM should be transferred to Cosmos Hub first and only after that to Neutron.

### Direct usage via SkipAPI

> SkipAPI integration is work in progress yet and will be available shortly after the launch

Instead of manually guiding SkipAPI on what you want to do with the ASSET (i.e. send to Neutron and stake) one can just specify the desired result and let SkipAPI itself decide on what's the proper way of converting assets.
This approach is flexible but might have potential caveat in dASSET being bought on a DEX instead of being staked.
...
Since dASSET is a native denom, one can get the dASSET balance by querying the bank module.
